if (/\/film\/\d+/.test(window.location.href)) {
    window.addEventListener('load', createMaterialWidget);
}

function createMaterialWidget() {
    const title = document.querySelector('meta[property="og:title"]')?.content.split(' (')[0] || "Неизвестный фильм";
    ...
    const mainRating = fullRating !== "—" ? fullRating.substring(0, 3) : "—";

    const duration = document.querySelector('[data-test-id="duration"] [class^="styles_value"]')?.textContent.trim() || "";

    const descriptionElement = document.querySelector('[class^="styles_topText"] p') ||
        document.querySelector('[data-tid="bfd38da2"]');
