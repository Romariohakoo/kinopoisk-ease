/*
 * Общие кирпичики интерфейса: DOM-хелпер, иконки, шрифты, стили для Shadow DOM, тема сайта, цвет постера.
 * Разметка собирается через createElement/textContent — данные со страницы никогда не попадают в innerHTML.
 */
(function (root) {
    'use strict';

    const { parseColor, luminance, pickAccent } = root.KPE;

    const SVG_NS = 'http://www.w3.org/2000/svg';

    // @font-face внутри Shadow DOM Chrome игнорирует, поэтому шрифты регистрируются на документе.
    // Семейство с префиксом KPE, чтобы не конфликтовать со шрифтами самого сайта.
    const FONTS = [
    ...
    const ICONS = {
        ...
        settings: {
            box: 21,
            paths: [{ d: GEAR, fill: 'currentColor', 'fill-rule': 'evenodd', 'clip-rule': 'evenodd' }],
        },
        star: {
            box: 13,
            paths: [
    ...
    function loadFonts() {
        ...
        for (const [file, weight] of FONTS) {
            ...
            const face = new FontFace('KPE Manrope', source, { weight, style: 'normal', display: 'swap' });
    ...
    function fetchSheet(path) {
        if (!sheets.has(path)) {
            ...
            promise.catch(() => sheets.delete(path));
            sheets.set(path, promise);
        }
        return sheets.get(path);
    }

    /** Подключает стили расширения к shadow root; промис выполняется, когда стили применены. */
    function adoptStyles(shadow, paths) {
        return Promise.all(paths.map(fetchSheet))
            .then((list) => {
                shadow.adoptedStyleSheets = list;
            })
            .catch(() => {
                for (const path of paths) {
                    shadow.prepend(h('link', { rel: 'stylesheet', href: chrome.runtime.getURL(path) }));
                }
            });
    }
    ...
    function icon(name, size) {
        const { box, paths } = ICONS[name];
        const svg = document.createElementNS(SVG_NS, 'svg');
        svg.setAttribute('width', size || box);
        svg.setAttribute('height', size || box);
    ...
    /** Светлая или тёмная страница — по фону первых непрозрачных контейнеров. */
    function detectPageTheme() {
        ...
        for (const el of candidates) {
            ...
            if (color && color.a > 0.5) return luminance(color) < 0.35 ? 'dark' : 'light';
        }
        return window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
    }

    function resolveTheme(setting) {
        return setting === 'dark' || setting === 'light' ? setting : detectPageTheme();
    }

    /**
     * Акцентный цвет постера. Картинку нужно прочитать через canvas, а это возможно только если CDN
     * отдаёт CORS-заголовки; иначе — null, и плашка остаётся в нейтральных цветах.
     */
                ...
                new Promise((resolve) => {
                    ...
                    img.onload = () => {
                        ...
                        try {
                            ...
                            resolve(pickAccent(context.getImageData(0, 0, 24, 24).data));
                        } catch {
                            resolve(null);
                        }
                    };
    ...
    /** Горячие клавиши сайта не должны перехватывать ввод внутри наших элементов. */
    ...
    root.KPE.ui = { h, icon, loadFonts, adoptStyles, resolveTheme, extractAccent, prefersReducedMotion, isolateKeys };
})(globalThis);
